...
func (s *DBStorage) CreateUser(ctx *fasthttp.RequestCtx, newUser *user.User) (*user.User, error) {
...
func (s *DBStorage) FindUserByToken(ctx *fasthttp.RequestCtx, token string) (*user.User, error) {
	conn, err := s.instance.Acquire(ctx)
	if err != nil {
		return nil, newDBError(err)
	}
